...
export default function Home() {
  ...
  const handleCreateTab = async (e: React.FormEvent) => {
    ...
    try {
      const slug = `${tabName.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "")}-${Math.random().toString(36).substring(2, 7)}`;

      // 1. Insert Tab
      const { data: tabData, error: tabError } = await supabase
        .from("tabs")
        .insert([{ title: tabName.trim(), slug }])
        .select("id, slug")
        .single();
      ...
      console.error(err);
      setErrorMsg(err.message || "Failed to create tab. Check your database setup.");
      setLoading(false);
    }
  };
  ...
  return (
    <main className="min-h-screen bg-[#0B0F17] text-slate-100 flex flex-col justify-center px-5 py-12 max-w-md mx-auto">
      ...
      <form onSubmit={handleCreateTab} className="space-y-5 bg-white/[0.03] border border-white/10 p-6 rounded-2xl backdrop-blur-md">
        ...
        >
          {loading ? <Loader2 size={18} className="animate-spin" /> : <>Create Shared Tab <ArrowRight size={16} /></>}
        </button>
      </form>
    </main>
  );
