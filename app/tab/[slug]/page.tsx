...
export default function TabPage() {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on("postgres_changes", { event: "*", schema: "public", table: "tabs" }, () => fetchTabData())
  ...
  const handleRenameTab = async (e: React.FormEvent) => {
    ...
    if (!tab || !cleanTitle || savingRename) return;
    ...
    try {
      ...
      setTab({ ...tab, title: cleanTitle });

      try {
        const existing = JSON.parse(localStorage.getItem("evenly_recent_tabs") || "[]");
        const updated = existing.map((item: { slug: string; title: string }) =>
          item.slug === tab.slug ? { ...item, title: cleanTitle } : item
        );
        localStorage.setItem("evenly_recent_tabs", JSON.stringify(updated));
      } catch (err) {
      ...
      alert(err.message || "Failed to rename tab");
    } finally {
      ...
      {isRenameModalOpen && (
        <div className="fixed inset-0 z-50 bg-black/50 dark:bg-black/75 backdrop-blur-sm flex items-end sm:items-center justify-center p-0 sm:p-4 animate-fade-in">
          <div className="w-full max-w-md bg-white dark:bg-[#121824] border-t sm:border border-black/10 dark:border-white/10 rounded-t-3xl sm:rounded-2xl p-6 space-y-4 animate-sheet-up shadow-2xl">
            <div className="flex justify-between items-center">
              <h3 className="text-base font-bold text-slate-900 dark:text-white flex items-center gap-2">
                <Pencil size={16} className="text-emerald-500" /> Rename Tab
              </h3>
              <button onClick={() => setIsRenameModalOpen(false)} className="text-slate-400 hover:text-slate-900 dark:hover:text-white p-1 active:scale-90 transition">
                <X size={20} />
              </button>
            </div>

            <form onSubmit={handleRenameTab} className="space-y-4">
              <div>
                <label className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase">
                  Tab Name
                </label>
              ...
              <button
                ...
                className="w-full bg-emerald-500 hover:bg-emerald-400 text-black font-bold py-3.5 rounded-xl text-sm active:scale-95 transition flex items-center justify-center gap-2"
              >
                {savingRename ? <Loader2 size={16} className="animate-spin" /> : "Save Changes"}
              </button>
            </form>
          </div>
        </div>
